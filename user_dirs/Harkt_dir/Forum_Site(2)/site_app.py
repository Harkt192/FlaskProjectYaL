...
def new_post():
    ...
    if form.validate_on_submit():
        db_sess = db_session.create_session()
        post = Post(title=form.title.data, content=form.content.data, user_id=current_user.id, author=current_user.username)
        db_sess.add(post)
        db_sess.commit()
        db_sess.close()
        return redirect(f'/{USER_LOGIN}/')
...
def post(post_id):
    try:
        db_sess = db_session.create_session()
        comments = db_sess.query(Comment).filter(Comment.post_id == post_id)
    ...
    if form.validate_on_submit():
        db_sess = db_session.create_session()
        comment = Comment(content=form.content.data, user_id=current_user.id, post_id=post_id, author=current_user.username)
        db_sess.add(comment)
        db_sess.commit()
        db_sess.close()
        return redirect(f'/{USER_LOGIN}/post/{post_id}')
